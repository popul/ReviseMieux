...
- **Ne jamais créer une issue GitHub sans labels.** Une issue sans tag est invisible dans les filtres (`is:open label:ci`, etc.) et brise le tri par domaine. Avant chaque `gh issue create`, lister les labels disponibles avec `gh label list` et en poser au moins un qui identifie le domaine (`ci`, `testing`, `infra`, `design`, `documentation`) et idéalement un qui identifie la nature (`bug`, `enhancement`, `question`). Si aucun label existant ne correspond, en créer un nouveau avec `gh label create` plutôt que de laisser l'issue orpheline. Historique : les issues #74, #75, #76, #78 ont été créées sans tags dans cette même session et ont dû être taguées rétroactivement.
