...
- **Ne jamais créer une issue GitHub sans labels.** Une issue sans tag est invisible dans les filtres (`is:open label:ci`, etc.) et brise le tri. Avant chaque `gh issue create`, lister les labels disponibles avec `gh label list` et poser **au minimum deux labels** :
  1. **Une dimension axe de valeur** — soit `technique` (dette, outillage, CI, tests, infra interne, refactoring — invisible pour l'utilisateur final), soit `fonctionnel` (valeur utilisateur directe, feature visible, UX). Cet axe permet de distinguer "ce qui fait avancer le produit" de "ce qui maintient la machine".
  2. **Un label de domaine** identifiant le périmètre (`ci`, `testing`, `infra`, `design`, `documentation`).

  Idéalement, ajouter aussi un label de nature (`bug`, `enhancement`, `question`). Si aucun label existant ne correspond à un besoin récurrent, en créer un nouveau avec `gh label create` plutôt que de laisser l'issue orpheline. Historique : les issues #74, #75, #76, #78 ont été créées sans tags dans cette même session et ont dû être taguées rétroactivement, puis l'axe `technique`/`fonctionnel` a dû être ajouté après coup sur l'ensemble du corpus parce que les labels de domaine ne permettaient pas de distinguer dette technique et valeur utilisateur.
